...
if not all_projects:
    all_projects = ["預設專案"]
    pd.DataFrame([]).to_csv(os.path.join(SAVE_DIR, "預設專案.csv"))
...
for i, row in df.iterrows():
    with st.sidebar.expander(f"📌 {row['階段']} ({row['狀態']})"):
        ...
        # 自動邏輯：若 100% 則自動完成，但排除要徑與問題狀態的手動標記
...
def highlight_status(val):
    if val == "已完成":
        ...
        return 'background-color: #fdecea; color: #c62828; font-weight: bold;'
    return ''
...
st.download_button("📥 匯出 PDF 報告", data=export_pdf(df), file_name=f"{current_project}_report.pdf")